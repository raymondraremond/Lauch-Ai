...
import { LayoutDashboard, Wand2, MessageSquare, Compass, Rocket, Settings, ChevronRight, Target, LogOut, X } from 'lucide-react'
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Backdrop for mobile */}
      <div 
        className={`fixed inset-0 bg-void/80 backdrop-blur-sm z-[45] lg:hidden transition-opacity duration-300 ${isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={closeSidebar}
      />

      <aside className={`fixed left-0 top-0 h-full w-[260px] lg:w-[220px] bg-base border-r border-dim flex flex-col z-50 pt-6 lg:pt-20 pb-6 font-body transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="flex items-center justify-between px-6 mb-8 lg:hidden">
            ...
            <button onClick={closeSidebar} className="p-2 text-text-muted hover:text-primary">
              <X size={20} />
            </button>
        </div>
        ...
        <div className="mx-3 space-y-2 mt-auto pt-4 border-t border-dim lg:border-none">
          {/* AI Status Badge */}
